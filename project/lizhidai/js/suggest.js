    ...
    $.fnSuggestLib = function(input, options) {
		...
		function ajaxSourceLib(val){
			...
			$.ajax({
				...
				success: function(data) {
					Util.config.CITYSPOTLIGHT.splice(0,Util.config.CITYSPOTLIGHT.length);
					$.each(data,function(key, items){
						Util.config.CITYSPOTLIGHT.push(items);
					});
				}
			});
		};
		...
		function hotSource(){
			...
			$.getJSON(options.hotUrl,function(data){
				$.each(data,function(key, items){
					Util.config.HOTCITYSPOTLIGHT.push(items);
				});
			});
		};
    ...
    $.fnBanksLib = function(input, options) {
		...
		function ajaxSourceLib(){
			...
			if(!(bank == '' || bank == null || bank == '未选择银行')){
				$.ajax({
					url: environment.basePath+'/rest/const/subbranch/'+encodeURI(bank)+'_'+bankcode+'?d='+new Date().getTime(),
					type: "GET",
					dataType: "json",
					success: function(data) {
						Util.config.BANKSSPOTLIGHT.splice(0,Util.config.BANKSSPOTLIGHT.length);
						$.each(data,function(key, items){
							Util.config.BANKSSPOTLIGHT.push(items);
						});
					}
				});
			}
...
$(function(){
	Util.loadCSS("css/suggest.css", "suggestListStyle");
	$('#BankCity').fnSuggestLib({
		//url: environment.basePath + '/rest/const/citypinyin?d='+new Date().getTime(),
		data: Util.config.CITYSPOTLIGHT,
		onSelect: function(){
			
		}
	});
	$('#HouseBankName').fnBanksLib({
		data: Util.config.BANKSSPOTLIGHT,
		width: 300,
		coder: false,
		onSelect: function(){
